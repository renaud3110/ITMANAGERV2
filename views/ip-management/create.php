...
<div class="container mt-4">
    <div class="page-header">
        <h1 class="page-title">Nouvelle Adresse IP</h1>
        ...
        <form method="POST" class="ip-form">
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" class="form-control" rows="3" 
                          placeholder="Description de l'utilisation de cette adresse IP..."></textarea>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Suggestions automatiques pour le masque de sous-réseau
    if (subnetInput) {
        subnetInput.addEventListener('focus', function() {
            if (!this.value) {
                this.placeholder = '255.255.255.0 (suggéré)';
            }
        });
    }
    
    // Auto-complétion de la passerelle basée sur l'IP
    if (ipInput && gatewayInput) {
        ipInput.addEventListener('blur', function() {
            if (this.value && !gatewayInput.value) {
                const ipParts = this.value.split('.');
                if (ipParts.length === 4) {
                    // Suggérer la première adresse du réseau comme passerelle
                    ipParts[3] = '1';
                    gatewayInput.placeholder = ipParts.join('.') + ' (suggéré)';
                }
            }
        });
    }
